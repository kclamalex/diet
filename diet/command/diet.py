...
@click.command()
@click.option("--rand", type=bool, default=False, is_flag=True, show_default=True)
@click.pass_context
def plan(ctx):
    food_list = []
    consumption_list = []
    ...
    all_proteins = food_repo.get_all(**{"food_type": "protein"})
    all_carbs = food_repo.get_all(**{"food_type": "carbs"})
